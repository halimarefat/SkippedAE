...
from utils.utils import OFLESDataset, R2Score, HEADERS, M1_HEADERS, M2_HEADERS, M3_HEADERS, M4_HEADERS
from model.skippedAE import skippedAE
from utils.loss import WaveletLoss

with open('datasets/coeffs/train/fieldData_R3_seen_means.txt', 'r') as file:
    data = [float(line.strip()) for line in file]
train_means = pd.DataFrame(np.reshape(data, (-1, len(HEADERS))), columns=HEADERS)

with open('datasets/coeffs/train/fieldData_R3_seen_scales.txt', 'r') as file:
    data = [float(line.strip()) for line in file]
...
M4 = train_norm.filter(M4_HEADERS, axis=1)
...
model.to(device)
model.double()
criterion = WaveletLoss(wavelet='haar') #nn.MSELoss()
...
with open(f"./logs/R103_training_history_{dt_name}.json", "w") as f:
    json.dump(history, f)
    
print(f"\n Training history saved to './logs/R103_training_history_{dt_name}.json'")

data_iter = iter(train_loader)
next(data_iter)[:,0:-1]

traced_script_module = torch.jit.trace(model, next(data_iter)[:,0:-1].to(device))
traced_script_module.save(f"./traced/R103_traced_model_{dt_name}.pt")

print(f"Traced model saved to './traced/R103_traced_model_{dt_name}.pt'")