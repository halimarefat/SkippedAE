...
num_epochs = 500
patience = 20
best_model_path = f'./checkpoints/R104_model_{dt_name}.pt'
out_channels = 1
...
with open(f"./logs/R104_training_history_{dt_name}.json", "w") as f:
    json.dump(history, f)
    
print(f"\n Training history saved to './logs/R104_training_history_{dt_name}.json'")

data_iter = iter(train_loader)
next(data_iter)[:,0:-1]

traced_script_module = torch.jit.trace(model, next(data_iter)[:,0:-1].to(device))
traced_script_module.save(f"./traced/R104_traced_model_{dt_name}.pt")

print(f"Traced model saved to './traced/R104_traced_model_{dt_name}.pt'")